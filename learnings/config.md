...
The obfuscated class names (`w50/h.java` in v3.19, `c60/c.java` etc in v3.20)
change between releases — the extractor greps for the header literal, not the
class name.
...
|---|---|---|
| `integrity_token` | Play Integrity verdict wrapped in a JWE by Twickets' backend | Ephemeral per app launch, stored in SharedPreferences `prosopo_protect/integrity_token` (~22h client re-mint timer, 24h max validity check). See [prosopo-protection.md](prosopo-protection.md) |
| `attestation signature` | Hardware-backed ECDSA signature per request (v3.20+) | Key lives in AndroidKeyStore/StrongBox; requires a live device. See [prosopo-protection.md](prosopo-protection.md) |

Live extraction of the integrity token requires a rooted emulator with Play
Integrity bypasses — see the
[twickets-key-extractor](https://github.com/ahobsonsayers/twickets-key-extractor)
repo. As of v3.20, requests may additionally require the hardware attestation
headers, which only work on a real Android environment with AndroidKeyStore.
